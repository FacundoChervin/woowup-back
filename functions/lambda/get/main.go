...
func init() {
	...
	dynadb.Initialize(awssession.GetSession())
	...
	emailsrepo := emailsrepo.CreateRepository(awssession.GetSession(), dynadb.GetInstance(), viper.GetString("LAMBDA_AWS_DYNAMO_TABLE"))
	...
	handler = mw.Build(emailshdl.Get)
}
